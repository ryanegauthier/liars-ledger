...
function initSidebar() {
  ...
  style.textContent = [
    "#ll-bar { position:fixed; bottom:0; left:0; right:0; z-index:2147483647;",
    ...
    "#ll-header { display:flex; align-items:center; padding:8px 16px; gap:12px; border-bottom:1px solid #2a2d35; }",
    "#ll-logo { font-size:13px; color:#c8a96e; letter-spacing:0.08em; white-space:nowrap; }",
    "#ll-topics { font-size:11px; color:#5a5855; flex:1; }",
    "#ll-topics span { color:#8a8680; margin-right:8px; }",
    "#ll-close { background:none; border:none; color:#5a5855; cursor:pointer; font-size:20px; line-height:1; padding:2px 6px; transition:color 0.15s; }",
    "#ll-close:hover { color:#c8a96e; }",
    "#ll-cards { display:flex; overflow-x:auto; scrollbar-width:thin; scrollbar-color:#2a2d35 transparent; }",
    ".ll-card { min-width:220px; max-width:280px; padding:12px 16px; border-right:1px solid #1a1d25; cursor:pointer; transition:background 0.15s; flex-shrink:0; }",
    ".ll-card:hover { background:#161920; }",
    ".ll-card.ll-active { background:#161920; border-top:2px solid #c8a96e; }",
    ".ll-card-name { font-size:14px; color:#e8e4d9; margin-bottom:2px; }",
    ".ll-card-meta { font-size:11px; color:#5a5855; margin-bottom:8px; }",
    ".ll-party-D { color:#5a8ac8; } .ll-party-R { color:#c85a5a; } .ll-party-I { color:#8a8680; }",
    ".ll-indicators { display:flex; gap:6px; flex-wrap:wrap; }",
    ".ll-indicator { font-size:11px; padding:2px 8px; border-radius:2px; white-space:nowrap; }",
    ".ll-indicator-green { background:#1e3a2f; color:#4caf82; border:1px solid #2a5a3f; }",
    ".ll-indicator-gray  { background:#1a1d25; color:#5a5855; border:1px solid #2a2d35; }",
    ".ll-card-claim { font-size:11px; color:#8a8680; line-height:1.35; margin-top:6px; font-style:italic; }",
    ".ll-not-found-card { min-width:160px; padding:12px 16px; border-right:1px solid #1a1d25; flex-shrink:0; opacity:0.4; }",
    ".ll-not-found-name { font-size:12px; color:#8a8680; margin-bottom:2px; }",
    ".ll-not-found-reason { font-size:10px; color:#3a3835; font-style:italic; }",
    "#ll-detail { border-top:1px solid #2a2d35; padding:12px 16px; max-height:160px; overflow-y:auto; display:none; scrollbar-width:thin; scrollbar-color:#2a2d35 transparent; }",
    "#ll-detail.ll-visible { display:block; }",
    ".ll-detail-title { font-size:12px; color:#c8a96e; margin-bottom:8px; }",
    ".ll-detail-claim { font-size:12px; color:#a8a49a; line-height:1.4; margin-bottom:10px; padding-bottom:8px; border-bottom:1px solid #1a1d25; }",
    ".ll-bill { padding:6px 0; border-bottom:1px solid #1a1d25; display:flex; gap:10px; align-items:flex-start; }",
    ".ll-bill:last-child { border-bottom:none; }",
    ".ll-bill-type { font-size:10px; color:#5a5855; white-space:nowrap; min-width:70px; padding-top:2px; }",
    ".ll-bill-title { font-size:12px; color:#e8e4d9; line-height:1.4; margin-bottom:2px; }",
  ...
  bar.innerHTML =
    '<div id="ll-header">' +
      '<div id="ll-logo">Liars Ledger</div>' +
      '<div id="ll-topics"></div>' +
      '<button id="ll-close" title="Close">\u00d7</button>' +
    '</div>' +
    '<div id="ll-cards"></div>' +
...
function renderSidebar(results) {
  ...
  var topicsEl = document.getElementById("ll-topics");
  var cardsEl  = document.getElementById("ll-cards");
  ...
  (results.records || []).forEach(function(record, idx) {
    ...
    cardsHTML +=
      '<div class="ll-card" data-idx="' + idx + '">' +
        '<div class="ll-card-name">' + p.display + "</div>" +
        '<div class="ll-card-meta"><span class="ll-party-' + partyCode + '">' + partyCode + "</span> &middot; " + p.state + " &middot; " + p.chamber + "</div>" +
  ...
  (results.notFound || []).forEach(function(name) {
    cardsHTML +=
      '<div class="ll-not-found-card">' +
        '<div class="ll-not-found-name">' + name + "</div>" +
  ...
  cardsEl.querySelectorAll(".ll-card").forEach(function(card) {
    card.addEventListener("click", function() {
      ...
      var wasActive = card.classList.contains("ll-active");

      cardsEl.querySelectorAll(".ll-card").forEach(function(c) {
        c.classList.remove("ll-active");
      });
      ...
      var html = '<div class="ll-detail-title">' + escapeHtml(p.full_name) + " &mdash; " + record.topics.map(escapeHtml).join(", ") + "</div>";
      ...
      if (allBills.length === 0) {
        ...
        allBills.forEach(function(bill) {
          ...
          var url = "https://www.congress.gov/bill/" + congress + "th-congress/" + typeName + "/" + number;
            html +=
            '<div class="ll-bill">' +
              '<div class="ll-bill-type">' + bill.role + "<br>" + (bill.type || "") + " " + (bill.number || "") + "</div>" +
              "<div>" +
                '<div class="ll-bill-title">' + escapeHtml(bill.title || "Untitled") + "</div>" +
                '<div class="ll-bill-date">' + (bill.introducedDate || "") +
                  ' &middot; <a class="ll-bill-link" href="' + url + '" target="_blank">View &rarr;</a>' +
                "</div>" +
              "</div>" +
            "</div>";
        });
...
function startPolling() {
  ...
  var poll = setInterval(function() {
    console.log("[Liars Ledger] polling...");
