...
- **Scan limits** are tracked per anonymous token in Redis (`scans:{tokenId}:{date}`), checked and incremented via `POST /api/scan/start`. As of the tier restructure that separated AI features from sourced-data features, the daily limit depends on tier: free tier scales down automatically as the registered-user count grows, pro tier gets a flat daily allowance separate from free's pool (see `server/providers/store.js`'s `getScanLimit`). This was a deliberate reversal of an earlier design decision, see CHANGELOG for the version where this changed.
...
- **Why this is acceptable given the actual blast radius**: the token is a random UUID, not predictable or guessable, and not reusable to extract anything beyond what it already grants - that token's own scan allowance (whatever its tier's limit is), the ability to subscribe that token to Pro, or `/restore-token` access (which itself requires possession of a real completed Square order reference). Worst case if a token leaks is someone else uses the associated scan allowance, or, paradoxically harmlessly, subscribes that token to Pro using their own card. No PII, payment data, or account access is tied to the token itself.
